...
public class CalculateCoordinateDistance extends AbstractTDBLeafTask { 
    @Getter @Setter
    @RDFSubject
    private String url;
    ...
    @Getter @Setter
    private BehaviorConstructQuery tcnLocation;

    @RDF("welcomePlugin:registrationOfficeLocation")
    @Getter @Setter
    private BehaviorConstructQuery registrationOfficeLocation;
    ...
    public String toString() {
        return "CalculateCoordinateDistance (" + url + ")";
    }
    ...
    private boolean performWrite() throws ConditionEvaluationException, URISyntaxException, QueryEvaluationException {
        ...
        if (tcnLocation.getTargetBase().toString().equals(AJANVocabulary.EXECUTION_KNOWLEDGE.toString())) {
            this.getObject().getExecutionBeliefs().update(model);
        } else if (tcnLocation.getTargetBase().toString().equals(AJANVocabulary.AGENT_KNOWLEDGE.toString())) {
    ...
    private Model findClosestOffice() throws URISyntaxException{
        Repository originTCNModel = BTUtil.getInitializedRepository(getObject(), tcnLocation.getOriginBase());
        Repository originOfficeModel = BTUtil.getInitializedRepository(getObject(), registrationOfficeLocation.getOriginBase());
        Model tcnModel = tcnLocation.getResult(originTCNModel);
        Model officeModel = registrationOfficeLocation.getResult(originOfficeModel);
