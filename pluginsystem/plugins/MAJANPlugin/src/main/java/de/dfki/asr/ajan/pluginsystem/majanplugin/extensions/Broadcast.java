 ...
 */
@Extension
@RDFBean("bt:Broadcast")
@SuppressWarnings("PMD.ExcessiveImports")
public class Broadcast extends AbstractTDBLeafTask implements NodeExtension, TreeNode{
	@Getter @Setter
	@RDFSubject
	private String url;
	...
	//protected String requestURI;
        protected List<String> requestUriList = new ArrayList<>();
	protected HttpConnection request;
	protected static final Logger LOG = LoggerFactory.getLogger(Broadcast.class);

	@Override
	public Resource getType() {
		return MAJANVocabulary.BROADCAST;
	}
	...
	protected boolean checkResponse(final Object response, String requestURI) throws URISyntaxException {
	...
	protected boolean updateBeliefs(final Model model, final URI targetBase) {
		if (targetBase.toString().equals(AJANVocabulary.EXECUTION_KNOWLEDGE.toString())) {
			...
			return true;
		} else if (targetBase.toString().equals(AJANVocabulary.LOCAL_SERVICES_KNOWLEDGE.toString())) {
			this.getObject().getLocalServicesBeliefs().update(model);
			return true;
		} else if (targetBase.toString().equals(AJANVocabulary.LOCAL_AGENTS_KNOWLEDGE.toString())) {
